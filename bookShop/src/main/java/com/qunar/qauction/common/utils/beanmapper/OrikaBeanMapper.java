...
/**
 * Description: A bean mapper which implements the  {@link BeanMapper}, and base on orika object mapping framework
 * <p> Also it support some enhanced features from orika by implements  {@link ma.glasnost.orika.MapperFacade}
 * @author lingtong.fu
 * @version 2016-04-20 22:52
 */
public class OrikaBeanMapper implements ApplicationContextAware,
    InitializingBean, MapperFacade {
    ...
    */
    protected void configureFactoryBuilder(
        DefaultMapperFactory.Builder factoryBuilder) {
    }

    /**
    * find out all the customize converters which already be registered as spring bean, then register them in MapperFactory
    */
    ...
    /**
    * find out all the customize mappers which already be registered as spring bean, then register them in MapperFactory
    */
    @SuppressWarnings({"rawtypes",
        "unchecked"
    })
        ...
        for (Class<?> clazz : clazzCollection) {
            ...
            if (StringUtils.isEmpty(mapClassAnnotation.value())) {
                throw new IllegalArgumentException(
                    "MapClass annotation do not have value in class: " + clazz +
                    "");
            }
            ...
            try {
                ...
                logger.warn("can not find mapping class [" +
                    mapClassAnnotation.value() + "] for [" + clazz + "]");

                continue;
            }

            ClassMapBuilder classMapBuilder = factory.classMap(clazz,
                    destinationClass);
            logger.info("create mapping: [" + clazz + "] === [" +
                destinationClass + "]");
            ...
            for (Field field : clazz.getDeclaredFields()) {
                if (field.isAnnotationPresent(MapField.class)) {
                    ...
                    if (StringUtils.isEmpty(mapFieldAnnotation.value()) &&
                        ...
                        throw new IllegalArgumentException(
                            "MapField annotation must have one of value and complexMap setting on Field: " +
                            field);
                    }

                    if (!StringUtils.isEmpty(mapFieldAnnotation.value()) &&
                            !StringUtils.isEmpty(
                                mapFieldAnnotation.complexMap())) {
                        throw new IllegalArgumentException(
    ...
    */
    ...
    public <S, D> void map(S sourceObject, D destinationObject,
        Type<S> sourceType, Type<D> destinationType, MappingContext context) {
        facade.map(sourceObject, destinationObject, sourceType,
    ...
    public <S, D> D map(S sourceObject, Type<S> sourceType,
    ...
    public <S, D> Set<D> mapAsSet(Iterable<S> source, Type<S> sourceType,
        Type<D> destinationType) {
    ...
    public <S, D> Set<D> mapAsSet(S[] source, Type<S> sourceType,
        Type<D> destinationType) {
        return facade.mapAsSet(source, sourceType, destinationType);
    }
    ...
    public <S, D> List<D> mapAsList(Iterable<S> source, Type<S> sourceType,
        Type<D> destinationType) {
    ...
    public <S, D> List<D> mapAsList(S[] source, Type<S> sourceType,
        Type<D> destinationType) {
        return facade.mapAsList(source, sourceType, destinationType);
    }
    ...
    public <S, D> D[] mapAsArray(D[] destination, Iterable<S> source,
        Type<S> sourceType, Type<D> destinationType) {
        return facade.mapAsArray(destination, source, sourceType,
            destinationType);
    }
    ...
    public <S, D> void mapAsCollection(Iterable<S> source,
        Collection<D> destination, Class<D> destinationClass) {
    ...
    public <S, D> void mapAsCollection(S[] source, Collection<D> destination,
        Class<D> destinationCollection) {
        facade.mapAsCollection(source, destination, destinationCollection);
    }

    public <S, D> void mapAsCollection(S[] source, Collection<D> destination,
        Class<D> destinationCollection, MappingContext context) {
        facade.mapAsCollection(source, destination, destinationCollection,
            context);
    }

    public <S, D> void mapAsCollection(Iterable<S> source,
        Collection<D> destination, Type<S> sourceType, Type<D> destinationType) {
    ...
    public <S, D> void mapAsCollection(Iterable<S> source,
        Collection<D> destination, Type<S> sourceType, Type<D> destinationType,
        MappingContext context) {
        facade.mapAsCollection(source, destination, sourceType,
            destinationType, context);
    }
    ...
    public <S, D> D convert(S source, Type<S> sourceType,
        Type<D> destinationType, String converterId) {
    ...
    public <S, D> D newObject(S source, Type<?extends D> destinationClass,
        MappingContext context) {
    ...
    public <S, Dk, Dv> Map<Dk, Dv> mapAsMap(Iterable<S> source,
        Type<S> sourceType, Type<?extends Map<Dk, Dv>> destinationType) {
        return facade.mapAsMap(source, sourceType, destinationType);
    }
    ...
    public <Sk, Sv, D> List<D> mapAsList(Map<Sk, Sv> source,
        Type<?extends Map<Sk, Sv>> sourceType, Type<D> destinationType) {
        return facade.mapAsList(source, sourceType, destinationType);
    }
    ...
    public <Sk, Sv, D> D[] mapAsArray(D[] destination, Map<Sk, Sv> source,
        Type<?extends Map<Sk, Sv>> sourceType, Type<D> destinationType) {
        return facade.mapAsArray(destination, source, sourceType,
            destinationType);
    }
    ...
    public <S, D> MappingStrategy resolveMappingStrategy(S sourceObject,
        java.lang.reflect.Type rawAType, java.lang.reflect.Type rawBType,
        boolean mapInPlace, MappingContext context) {
        return facade.resolveMappingStrategy(sourceObject, rawAType, rawBType,
            mapInPlace, context);
    }
    ...
    public <A, B> BoundMapperFacade<A, B> dedicatedMapperFor(Class<A> aType,
        Class<B> bType, boolean containsCycles) {
